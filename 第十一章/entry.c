...
void kern_init()
{
	...
	//测试用的一个虚拟地址
	uint32_t allc_va = 0xC0000;
	uint32_t get_map_check;
	...
	printk("\n****after map***\n");
	map(pgd_kern, allc_va, allc_addr, 0);
	printk("the visual address is 0x%x\n",allc_va);
	
	printk("\n***get_mapping check***\n");
	get_mapping(pgd_kern, allc_va, &get_map_check);
	printk("get_mapping check is 0x%x\n",get_map_check);
