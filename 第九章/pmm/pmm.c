...
void init_pmm()
{
	//把multiboot的信息传递给了mmap
	mmap_entry_t *mmap_start_addr = (mmap_entry_t *)glb_mboot_ptr->mmap_addr;
	mmap_entry_t *mmap_end_addr = (mmap_entry_t *)glb_mboot_ptr->mmap_addr + glb_mboot_ptr->mmap_length;
	
	//指针变量，在for循环里用作循环指示，对mmap里的内存段进行遍历
	mmap_entry_t *map_entry;

	//for循环，遍历我们上一章给出的mmap
	for (map_entry = mmap_start_addr; map_entry < mmap_end_addr; map_entry++) {
		...
		if (map_entry->type == 1 && map_entry->base_addr_low == 0x100000) {
			...
			//while循环，用来在当前地址空间进行计数，按照4K页大小统计，把信息写入我们的phy_page_count里
...
{
	error_detect(pmm_stack_top != 0, "out of memory");
...
{
	error_detect(pmm_stack_top != PAGE_MAX_SIZE, "out of pmm_stack");
